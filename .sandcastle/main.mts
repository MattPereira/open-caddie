...
// Where pnpm's store lives inside the sandbox. Must match
// npm_config_store_dir in .sandcastle/Dockerfile.
const SANDBOX_PNPM_STORE = "/home/agent/.pnpm-store";

// The copied node_modules records the host's store path in .modules.yaml.
// pnpm treats a modules directory belonging to a different store as stale and
// wants to purge it — non-interactively that is a hard failure
// (ERR_PNPM_ABORTED_REMOVE_MODULES_DIR_NO_TTY), so repoint it at the sandbox
// store first. The captured group keeps the store version suffix (/v10) as-is.
const repointPnpmStore = `node -e 'const fs=require("fs"),f="node_modules/.modules.yaml";fs.existsSync(f)&&fs.writeFileSync(f,fs.readFileSync(f,"utf8").replace(/^storeDir: .*(\\/v\\d+)$/m,"storeDir: ${SANDBOX_PNPM_STORE}$1"))'`;

// Hooks run before the agent starts each iteration: the host hook after
// copyToWorktree, the sandbox hook once the container is up. pnpm install
// resolves to a no-op when the copied node_modules already matches the
// lockfile, and otherwise fetches only what is missing.
const hooks = {
  host: { onWorktreeReady: [{ command: repointPnpmStore }] },
