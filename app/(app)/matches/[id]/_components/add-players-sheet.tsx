...
import { addPlayersToMatch } from "../../actions";
...
export function AddPlayersSheet({
  ...
  const onSubmit = () => {
    if (selectedIds.size === 0 || selectedTeeId === "") return;
    setServerError(null);
    startTransition(async () => {
      const result = await addPlayersToMatch({
        matchId,
        teeId: selectedTeeId,
    ...
    <Sheet open={open} onOpenChange={handleOpenChange}>
      <SheetTrigger asChild>
        <Button variant="ghost" size="icon" aria-label="Add players">
          <HugeiconsIcon icon={UserAdd02Icon} aria-hidden />
        </Button>
      </SheetTrigger>
      <SheetContent
        side="right"
      ...
      >
        <SheetHeader className="shrink-0">
          ...
          <SheetDescription>Select players to add to this match</SheetDescription>
        </SheetHeader>

        <div className="flex min-h-0 flex-1 flex-col gap-3 px-4 pb-4">
          {players.length === 0 ? (
            <p className="rounded-md border border-dashed p-6 text-center text-sm text-muted-foreground">
              All app users are already in this match.
            </p>
          ) : (
            <>
              <select
                className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-base shadow-xs outline-none transition-[color,box-shadow] focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] disabled:cursor-not-allowed disabled:opacity-50 md:text-sm"
                disabled={tees.length === 0}
                value={selectedTeeId === "" ? "" : String(selectedTeeId)}
                onChange={(e) =>
                  setSelectedTeeId(
        ...
        {players.length > 0 ? (
          <SheetFooter className="shrink-0">
            <Button
              ...
              onClick={onSubmit}
              disabled={
                count === 0 || selectedTeeId === "" || tees.length === 0 || isPending
              }
            >
              {isPending
                ? "Adding..."
